...
try:
    ...
    print('Warning: Cannot import psycopg_pool. Multithreading is disabled.')
    pg_pool = NotImplemented
...
def get_hostname():
    res = subprocess.run('hostname', shell=False, capture_output=True, text=True)
...
class Postgres:
    def __init__(
        ...
        *,
        ...
        self._auto_save_mode = auto_save_mode
    ...
    def __auto_save_path(self):
        if self._auto_save_mode == 'db':
            return 'cache.postgres_cache' if self._auto_save_path is None else self._auto_save_path
        else:
            # self._auto_save_mode == 'file'
            host = urllib.parse.quote_plus(self._host)
            return f'.{self._dbname}@{host}.{self._port}.cache.pkl' if self._auto_save_path is None else self._auto_save_path

    def _db_check_cache_table(self):
        assert self._auto_save_mode == 'db'
        res = self.__auto_save_path
        schema_index = res.find('.')
        if schema_index >= 0:
            schema, table = res[:schema_index], res[schema_index + 1:]
        else:
            schema, table = None, res
        if schema is not None:
            schema_existence = self.execute(f"select nspname from pg_namespace where nspname = '{schema}'")
            if not schema_existence:
                # empty result
                self.execute(f"create schema {schema}", fetch=False)
                self.commit()
        self.execute(f"""create table if not exists {table} (
            key varchar(4096) not null,
            value text not null,
            create_time timestamp default current_timestamp,
            primary key (dbname, host, port, key)
)""", fetch=False)
        self.commit()
        return res
    ...
    def _cache_backup(self):
        ...
        if self._auto_save_mode == 'db':
            ...
            target_table = self._db_check_cache_table()
            values = []
            for key, value in self._auto_save_cache.items():
                ...
                values.append(f"("
                              ...
                              f")")
            joined_values = ',\n'.join(values)
            sql = (f"insert into {target_table} (key, value) "
    ...
    def __cache_load(self):
        ...
        if self._auto_save_mode == 'db':
            ...
            target_table = self._db_check_cache_table()
            res = self.execute(f"select key, value from {target_table}")
            cache_dict = {}
            ...
            self._auto_save_cache = cache_dict
        else:
    ...
    def get_settings(self, key):
        ...
        try:
            ...
            self.refresh_cursor(pre_settings=True)
            self.__cur.execute(f"show {key};")
    ...
    def set_settings(self, key, value):
        ...
        if self._connection is not None:
            try:
                self.__cur.execute(f"set {key} = {value};")
            except Exception as e:
                self.refresh_cursor(pre_settings=True)
    ...
    def refresh_cursor(self, pre_settings=True):
    ...
    def get_connection_pool(self, max_size, min_size=1, open=True, num_workers=3):
        pool = pg_pool.ConnectionPool(
            kwargs=self.__db_args[1],
            min_size=min_size,
            max_size=max_size,
            open=open,
    ...
    def __ensure_execute(self, commands : Iterable, *args, pre_settings=True, retry_limit=None, **kwargs):
        ...
        for i in range(retry_limit):
            try:
                ...
                self.refresh_cursor(pre_settings=pre_settings)
        else:
    ...
    def execute(self, sql, cache=False, retry_limit=None, fetch=True):
        assert self._connection is not None
    ...
    def selectivity(self, table, where, explain=True, detail=False):
        ...
        if explain:
            ...
            select_rows = self.execute(f'select count(*) from {table} where {where};', cache=True)[0][0]
    ...
    def cost(self, sql, cache=True, throw=False):
        try:
            ...
            try:
                self._connection.commit()
            except psycopg.errors.DatabaseError as f:
                self.refresh_cursor()
            finally:
                if throw:
                    raise e
...
AND tc.table_name = c.table_name AND ccu.column_name = c.column_name WHERE constraint_type = 'PRIMARY KEY'"""
    ...
    def latency(self, sql, origin=None, return_plan=False, cache=True, throw=False):
        ...
        if origin is None:
            ...
            cost_ratio = cost / cost_origin if cost is not None and cost_origin is not None else None
...
class Schema:
    def __init__(self, database : Postgres = None):
        if database is not None:
            ...
            for i, (sname, tname) in enumerate(tables):
                ...
                table_db_indexes = {}
                self.table_db_indexes[tname] = table_db_indexes
                ...
                for index_name, index_def in res:
                    ...
                    if match:
                        ...
                        table_db_indexes[index_name] = columns
                        for i, c in enumerate(columns):
                            c = (sname, tname, c)
                            assert c in self.column_db_indexes
                            self.column_db_indexes[c].append((index_name, index_def, columns, i))
    ...
    def load_state_dict(self, state_dict):
        ...
        for name in (
            ...
            'column_db_indexes', 'table_db_indexes',
        ):
