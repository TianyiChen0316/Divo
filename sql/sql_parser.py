...
try:
    ...
    print('Warning: cannot import psqlparse', file=sys.stderr)
    class _psqlparse:
        @classmethod
        def parse_dict(cls, *args, **kwargs):
            raise NotImplementedError

import pathlib as _pathlib
from pathlib import PosixPath as _PosixPath, PurePosixPath as _PurePosixPath

try:
    _PosixPath()
except NotImplementedError:
    print('Warning: cannot use PosixPath. Trying to replace it with WindowsPath', file=sys.stderr)
...
class Element:
    ...
    def clone(self):
        # we use the constructor function instead of object.__new__(self.__class__) to initialize the fields with
        # default values
        res = self.__class__()
        for field in filter(
            lambda x: not callable(getattr(self, x))
                and not x.startswith('__')
                # the copied field cannot be a property
                and getattr(getattr(self.__class__, x, None), '__get__', None) is None,
            dir(self),
        ):
...
def sql_expr_analyze(arg, env : typing.Optional[ParserEnvironment] = None) -> Element:
    """
...
class NullTest(Element):
    ...
    @property
    def is_simple_filter(self):
        return isinstance(self.element, ColumnRef)

    @property
    def is_simple_eqjoin(self):
        return False
...
class ColumnRef(Element):
    ...
    @property
    def concerned_columns(self):
        return {(self.alias, self.column_name)} if self.alias or self.column_name else set()
...
class TypeCast(Element):
    ...
    def setup(self, args : dict, env = None):
        ...
        self.array_bounds = None
        if 'arrayBounds' in type_name:
            self.array_bounds = []
            for dic in type_name['arrayBounds']:
                assert 'Integer' in dic, dic
                assert 'ival' in dic['Integer'], dic
                self.array_bounds.append(dic['Integer']['ival'])
    ...
    def __str__(self):
        ...
        res = str(self.arg) + '::' + type_name + type_args
        if self.array_bounds is not None:
            array_bounds = []
            for val in self.array_bounds:
                array_bounds.append('[' + (str(val) if val >= 0 else '') + ']')
            res += ''.join(array_bounds)
        return res
...
class MathExpr(Element):
    ...
    BETWEEN = 10
    NOT_BETWEEN = 11
    BETWEEN_SYMMETRIC = 12
    NOT_BETWEEN_SYMMETRIC = 13
    ...
    def to_str(self, oracle=False):
        ...
        if self.kind == self.BETWEEN_SYMMETRIC:
            return f'{lexpr} BETWEEN SYMMETRIC {rexpr}'
        if self.kind == self.NOT_BETWEEN_SYMMETRIC:
            return f'{lexpr} NOT BETWEEN SYMMETRIC {rexpr}'
        if self.kind == self.ARITHMETIC_ANY:
            return f'{lexpr} {self.name} ANY ({rexpr})'
        if self.kind == self.ARITHMETIC_ALL:
            return f'{lexpr} {self.name} ALL ({rexpr})'
    ...
    def is_simple_filter(self):
        if isinstance(self.lexpr, list):
            return False
        if isinstance(self.rexpr, list):
            if self.kind not in (self.BETWEEN, self.NOT_BETWEEN):
                return False
            return isinstance(self.lexpr, ColumnRef) and not self.rexpr[0].concerned_columns and not self.rexpr[1].concerned_columns
        return isinstance(self.lexpr, ColumnRef) and not self.rexpr.concerned_columns \
            or isinstance(self.rexpr, ColumnRef) and not self.lexpr.concerned_columns

    @property
    def is_simple_eqjoin(self):
        return isinstance(self.lexpr, ColumnRef) and isinstance(self.rexpr, ColumnRef) \
            and self.kind == self.ARITHMETIC and self.name == '=' \
            and not (self.lexpr.alias == self.rexpr.alias and self.lexpr.column_name == self.rexpr.column_name)
...
class FuncCall(Element):
    GROUPING_FUNCS = ("any_value", "array_agg", "avg", "bit_and", "bit_or", "bit_xor", "bool_and", "bool_or", "count", "every", "json_agg", "jsonb_agg", "json_agg_strict", "jsonb_agg_strict", "json_arrayagg", "json_objectagg", "json_object_agg", "jsonb_object_agg", "json_object_agg_strict", "jsonb_object_agg_strict", "json_object_agg_unique", "jsonb_object_agg_unique", "json_object_agg_unique_strict", "jsonb_object_agg_unique_strict", "max", "min", "range_agg", "range_intersect_agg", "string_agg", "sum", "xmlagg", "corr", "covar_pop", "covar_samp", "regr_avgx", "regr_avgy", "regr_count", "regr_intercept", "regr_slope", "regr_sxx", "regr_sxy", "regr_syy", "stddev", "stddev_pop", "stddev_samp", "variance", "var_pop", "var_samp", "mode", "percentile_cont", "percentile_disc", "rank", "dense_rank", "percent_rank", "cume_dist", "grouping",)
    ...
    def is_grouping(self):
        return self.class_name is None and self.name is not None and self.name.lower() in self.GROUPING_FUNCS
...
class Operators(IntEnum):
    ...
    @classmethod
    def from_hint_name(cls, value : str, oracle=False):
        value = value.lower()
        if oracle:
            return {
                'use_nl': cls.nested_loop_join,
                'use_merge': cls.merge_join,
                'use_hash': cls.hash_join,
                'no_use_nl': cls.no_nested_loop_join,
                'no_use_merge': cls.no_merge_join,
                'no_use_hash': cls.no_hash_join,
                'leading': cls.leading,
            }.get(value, cls.default)
        return {
            'nestloop': cls.nested_loop_join,
            'mergejoin': cls.merge_join,
            'hashjoin': cls.hash_join,
            'nonestloop': cls.no_nested_loop_join,
            'nomergejoin': cls.no_merge_join,
            'nohashjoin': cls.no_hash_join,
            'leading': cls.leading,
        }.get(value, cls.default)
...
class PlanParser:
    def __init__(self, plan : dict):
        ...
        self._parse(plan)
        self._bushy_level = sum((1 if isinstance(x[0], str) and isinstance(x[1], str) else 0) for x in self.join_order)

    @property
    def bushy_level(self):
        return self._bushy_level
    ...
    def to_plan_indices(self, plan):
        ...
        return to_plan_indices

    def plan_attributes(self, plan):
        """
        Map attributes to plan nodes. 'plan' is a PlanBase object.
        """
        to_plan_indices = self.to_plan_indices(plan)
        res = {}
        for alias, plan_alias in to_plan_indices.items():
            res[plan_alias] = self._attrs[alias]
        return copy.deepcopy(res)

    def rest_join_orders(self, plan):
        to_plan_indices = self.to_plan_indices(plan)
        join_order = []
        for branch_node in self._nodes_join_order():
            parent, left, right, join = branch_node.index, branch_node.left_child, branch_node.right_child, branch_node.join_method
            join_order.append((left, right, join))
        new_join_order = []
        plan_new_node_index = plan.total_branch_nodes
        plan_parents = {k : plan.parent[k] if plan.parent[k] is not None else k for k in (*self._aliases, *range(plan_new_node_index))}
        for left, right, join_method in join_order:
            plan_left, plan_right = to_plan_indices.get(left, None), to_plan_indices.get(right, None)
            assert plan_left is not None and plan_right is not None
            plan_left_parent, plan_right_parent = plan_parents.get(plan_left, plan_left), plan_parents.get(plan_right, plan_right)
            if plan_left_parent == plan_left and plan_right_parent == plan_right:
                this_parent = self._parent.get(left, None)
                assert this_parent is not None and this_parent == self._parent.get(right, None)
                to_plan_indices[this_parent] = plan_new_node_index
                new_join_order.append((plan_left, plan_right, join_order))
                plan_parents[plan_left_parent] = plan_new_node_index
                plan_parents[plan_right_parent] = plan_new_node_index
                plan_new_node_index += 1
            else:
                assert plan_left_parent != plan_left and plan_right_parent != plan_right
                pass
        assert len(new_join_order) + plan.total_branch_nodes == len(self._aliases) - 1
        return tuple(new_join_order)
