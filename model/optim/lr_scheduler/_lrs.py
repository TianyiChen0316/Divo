...
from torch.optim.lr_scheduler import (
    LambdaLR, LinearLR, StepLR, MultiStepLR, CyclicLR,
    PolynomialLR, ExponentialLR, MultiplicativeLR, OneCycleLR,
...
class MultiStepConstantLR(_LRScheduler):
    ...
    def get_lr(self):
        ...
        left_milestone_index = bisect_left(self.milestones, self.last_epoch)
        if milestone_index == 0 or self.milestones[milestone_index - 1] != self.last_epoch:
            return [group['lr'] for group in self.optimizer.param_groups]
        gamma = self.gammas[milestone_index] / self.gammas[left_milestone_index]
...
class FuncWrappedLR:
    def __init__(self, scheduler, func: typing.Optional[typing.Callable[[float], float]] = None):
        self._scheduler = scheduler
        self._func = func
        self._lr_step()

    def _lr_step(self, print=False, epoch=None):
        self._last_lrs = [param_group['lr'] for param_group in self._scheduler.optimizer.param_groups]
        if self._func:
            for i, (last_lr, param_group) in enumerate(zip(self._last_lrs, self._scheduler.optimizer.param_groups)):
                lr = self._func(last_lr)
                param_group['lr'] = lr
                if print:
                    self.print_lr(self._scheduler.verbose, i, lr, epoch)

    def _lr_recover(self):
        for last_lr, param_group in zip(self._last_lrs, self._scheduler.optimizer.param_groups):
    ...
    def func(self, value):
        ...
        if not callable(value):
            raise ValueError(f"'{value.__class__.__name__}' object is not callable")
        self._func = value
    ...
    def get_last_lr(self):
        ...
        return [self._func(x) for x in self._scheduler.get_last_lr()]

    def get_lr(self):
        return self._scheduler.get_lr()

    def print_lr(self, is_verbose, group, lr, epoch=None):
        return self._scheduler.print_lr(is_verbose, group, lr, epoch=epoch)

    def step(self, epoch=None):
        verbose, self._scheduler.verbose = self._scheduler.verbose, False
        self._lr_recover()
        res = self._scheduler.step(epoch)
        self._scheduler.verbose = verbose
        self._lr_step(print=True, epoch=epoch)
        return res


__all__ = [
    *filter(lambda x: x.endswith('LR'), globals()),
    'ReduceLROnPlateau',
]
