--dsb_templates_query099_098_c2d8dc99-dbb6-3496-b547-49a9c8343b68.sql
--{"gen": "combine", "time": 3.2817325592041016, "template": "query099_098", "dataset": "dsb_templates", "rows": 6}
...
WHERE (call_center.cc_class = 'medium' AND catalog_sales.cs_list_price BETWEEN 257 AND 286 AND date_dim.d_month_seq BETWEEN 1212 AND 1212 + 23 AND ship_mode.sm_type = 'NEXT DAY' AND warehouse.w_gmt_offset = -5 AND item.i_category = 'Shoes' AND customer.c_birth_month = 2 AND store_sales.ss_wholesale_cost BETWEEN 9 AND 29 AND catalog_sales.cs_call_center_sk = call_center.cc_call_center_sk AND catalog_sales.cs_ship_date_sk = date_dim.d_date_sk AND catalog_sales.cs_ship_mode_sk = ship_mode.sm_ship_mode_sk AND catalog_sales.cs_warehouse_sk = warehouse.w_warehouse_sk AND date_dim.d_date_sk = store_sales.ss_sold_date_sk AND store_sales.ss_item_sk = item.i_item_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_sales.ss_store_sk = store.s_store_sk AND catalog_sales.cs_ship_date_sk = store_sales.ss_sold_date_sk)
