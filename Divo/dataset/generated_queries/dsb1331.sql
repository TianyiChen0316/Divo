--dsb_templates_query050_013_e38c99f4-3a4c-32a0-8b3b-d5c1db26dba2.sql
--{"gen": "combine", "time": 0.30673885345458984, "template": "query050_013", "dataset": "dsb_templates", "rows": 12}
SELECT store.s_city,
store.s_company_id,
store.s_county,
store.s_state,
...
WHERE (d2.d_moy = 2 AND d2.d_year = 2002 AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_sales.ss_store_sk = store.s_store_sk AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date)
