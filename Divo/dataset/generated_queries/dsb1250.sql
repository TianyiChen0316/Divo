--dsb_templates_generated-f9c9b0d7-1a18-484a-8b97-24c2ad067fb8_ea0e21c4-2bbe-380e-a21d-9ece7ff5354b.sql
--{"gen": "combine", "time": 1.5015029907226562, "template": "generated-f9c9b0d7-1a18-484a-8b97-24c2ad067fb8", "dataset": "dsb_templates", "rows": 1}
...
item AS item,
store AS store
WHERE (call_center.cc_class = 'small' AND catalog_sales.cs_list_price BETWEEN 51 AND 80 AND date_dim.d_month_seq BETWEEN 1187 AND 1187 + 23 AND ship_mode.sm_type = 'LIBRARY' AND warehouse.w_gmt_offset = -5 AND customer.c_birth_month = 11 AND customer_address.ca_state = 'MO' AND item.i_category = 'Sports' AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND catalog_sales.cs_call_center_sk = call_center.cc_call_center_sk AND catalog_sales.cs_ship_date_sk = date_dim.d_date_sk AND catalog_sales.cs_ship_mode_sk = ship_mode.sm_ship_mode_sk AND catalog_sales.cs_warehouse_sk = warehouse.w_warehouse_sk AND customer.c_current_addr_sk = customer_address.ca_address_sk AND date_dim.d_date_sk = store_sales.ss_sold_date_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_sales.ss_item_sk = item.i_item_sk AND store_sales.ss_store_sk = store.s_store_sk AND catalog_sales.cs_ship_date_sk = store_sales.ss_sold_date_sk AND store_returns.sr_item_sk = item.i_item_sk AND date_dim.d_date_sk = d1.d_date_sk AND d1.d_date_sk = catalog_sales.cs_ship_date_sk AND customer.c_customer_sk = store_returns.sr_customer_sk AND pg_catalog.substring(customer_address.ca_zip, 1, 5) <> pg_catalog.substring(store.s_zip, 1, 5))
 LIMIT 100