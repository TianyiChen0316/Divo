--dsb_templates_generated-5f0f5699-4051-4a38-99a2-7d35e0eeff28_78751d48-3c33-3b89-b4b3-f22c8a43763d.sql
--{"gen": "combine", "time": 1.5394444465637207, "template": "generated-5f0f5699-4051-4a38-99a2-7d35e0eeff28", "dataset": "dsb_templates", "rows": 1}
...
item AS item,
store AS store
WHERE (d2.d_year = 2000 AND d2.d_moy = 6 AND d3.d_moy BETWEEN 4 AND 4 + 2 AND d3.d_year = 2001 AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND customer.c_current_cdemo_sk = customer_demographics.cd_demo_sk AND inventory.inv_date_sk = store_sales.ss_sold_date_sk AND inventory.inv_item_sk = store_sales.ss_item_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_returns.sr_item_sk = inventory.inv_item_sk AND inventory.inv_warehouse_sk = warehouse.w_warehouse_sk AND inventory.inv_date_sk = d1.d_date_sk AND customer.c_customer_sk = store_returns.sr_customer_sk AND item.i_item_sk = store_sales.ss_item_sk AND store.s_store_sk = store_sales.ss_store_sk AND store_returns.sr_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = catalog_sales.cs_item_sk AND catalog_sales.cs_sold_date_sk = d3.d_date_sk AND store_sales.ss_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = store_sales.ss_item_sk AND item.i_item_sk = inventory.inv_item_sk AND inventory.inv_item_sk = catalog_sales.cs_item_sk AND customer.c_customer_sk = catalog_sales.cs_bill_customer_sk AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date AND inventory.inv_quantity_on_hand >= store_sales.ss_quantity)
 LIMIT 100