--dsb_templates_generated-a9f724e4-c857-4dd4-b931-e41b88dfe93b_d03c2b18-ec71-305f-951f-5e297f5ed508.sql
--{"gen": "erase", "time": 3.095710039138794, "template": "generated-a9f724e4-c857-4dd4-b931-e41b88dfe93b", "dataset": "dsb_templates", "rows": 9}
...
WHERE (call_center.cc_class = 'medium' AND catalog_sales.cs_list_price BETWEEN 188 AND 217 AND date_dim.d_month_seq BETWEEN 1186 AND 1186 + 23 AND ship_mode.sm_type = 'LIBRARY' AND warehouse.w_gmt_offset = -5 AND customer.c_birth_month = 8 AND customer_address.ca_state = 'IA' AND item.i_category = 'Electronics' AND store_sales.ss_wholesale_cost BETWEEN 13 AND 33 AND catalog_sales.cs_call_center_sk = call_center.cc_call_center_sk AND catalog_sales.cs_ship_date_sk = date_dim.d_date_sk AND catalog_sales.cs_ship_mode_sk = ship_mode.sm_ship_mode_sk AND catalog_sales.cs_warehouse_sk = warehouse.w_warehouse_sk AND customer.c_current_addr_sk = customer_address.ca_address_sk AND date_dim.d_date_sk = store_sales.ss_sold_date_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_sales.ss_item_sk = item.i_item_sk AND store_sales.ss_store_sk = store.s_store_sk AND catalog_sales.cs_ship_date_sk = store_sales.ss_sold_date_sk AND pg_catalog.substring(customer_address.ca_zip, 1, 5) <> pg_catalog.substring(store.s_zip, 1, 5))
