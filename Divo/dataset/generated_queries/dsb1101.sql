--dsb_templates_query050_013_61d3bd3f-1591-3a85-95bd-a94384fa1286.sql
--{"gen": "erase", "time": 0.367962121963501, "template": "query050_013", "dataset": "dsb_templates", "rows": 1}
...
WHERE (d2.d_year = 2000 AND d2.d_moy = 10 AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date)
 LIMIT 100