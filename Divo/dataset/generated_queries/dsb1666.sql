--dsb_templates_generated-5192e4d5-79f2-4018-8803-132dae96aae7_3d2fc52d-867e-3baa-aabd-ebae0ea157f1.sql
--{"gen": "combine", "time": 0.7983317375183105, "template": "generated-5192e4d5-79f2-4018-8803-132dae96aae7", "dataset": "dsb_templates", "rows": 1}
...
item AS item,
store AS store
WHERE (d2.d_year = 2002 AND d2.d_moy = 1 AND d3.d_moy BETWEEN 4 AND 4 + 2 AND d3.d_year = 2000 AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND item.i_item_sk = store_sales.ss_item_sk AND store.s_store_sk = store_sales.ss_store_sk AND store_returns.sr_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = catalog_sales.cs_item_sk AND catalog_sales.cs_sold_date_sk = d3.d_date_sk AND store_sales.ss_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = store_sales.ss_item_sk AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date)
 LIMIT 100