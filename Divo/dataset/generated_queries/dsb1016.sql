--dsb_templates_generated-a9f724e4-c857-4dd4-b931-e41b88dfe93b_a274c427-8c98-355f-ba3e-4270e459a7c1.sql
--{"gen": "combine", "time": 2.5976738929748535, "template": "generated-a9f724e4-c857-4dd4-b931-e41b88dfe93b", "dataset": "dsb_templates", "rows": 9}
...
WHERE (call_center.cc_class = 'large' AND catalog_sales.cs_list_price BETWEEN 158 AND 187 AND date_dim.d_month_seq BETWEEN 1211 AND 1211 + 23 AND ship_mode.sm_type = 'EXPRESS' AND warehouse.w_gmt_offset = -5 AND customer.c_birth_month = 4 AND customer_address.ca_state = 'WI' AND item.i_category = 'Shoes' AND store_sales.ss_wholesale_cost BETWEEN 17 AND 37 AND catalog_sales.cs_call_center_sk = call_center.cc_call_center_sk AND catalog_sales.cs_ship_date_sk = date_dim.d_date_sk AND catalog_sales.cs_ship_mode_sk = ship_mode.sm_ship_mode_sk AND catalog_sales.cs_warehouse_sk = warehouse.w_warehouse_sk AND customer.c_current_addr_sk = customer_address.ca_address_sk AND date_dim.d_date_sk = store_sales.ss_sold_date_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_sales.ss_item_sk = item.i_item_sk AND store_sales.ss_store_sk = store.s_store_sk AND catalog_sales.cs_ship_date_sk = store_sales.ss_sold_date_sk AND pg_catalog.substring(customer_address.ca_zip, 1, 5) <> pg_catalog.substring(store.s_zip, 1, 5))
