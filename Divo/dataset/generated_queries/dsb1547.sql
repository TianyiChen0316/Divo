--dsb_templates_query050_013_f6e28591-e85a-3521-81bb-547f5da238aa.sql
--{"gen": "erase", "time": 0.31887030601501465, "template": "query050_013", "dataset": "dsb_templates", "rows": 12}
SELECT store.s_city,
store.s_company_id,
store.s_county,
store.s_state,
...
WHERE (d2.d_year = 1998 AND d2.d_moy = 5 AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_store_sk = store.s_store_sk)
