--dsb_templates_query050_013_3fededf9-b4ab-3073-a1b5-b290cb133b83.sql
--{"gen": "combine", "time": 0.26346278190612793, "template": "query050_013", "dataset": "dsb_templates", "rows": 12}
SELECT store.s_city,
store.s_company_id,
store.s_county,
store.s_state,
...
WHERE (d2.d_moy = 7 AND d2.d_year = 2000 AND income_band.ib_lower_bound >= 22553 AND income_band.ib_upper_bound <= 40708 + 50000 AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_sales.ss_store_sk = store.s_store_sk AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND income_band.ib_income_band_sk = household_demographics.hd_income_band_sk AND customer_demographics.cd_demo_sk = customer.c_current_cdemo_sk AND household_demographics.hd_demo_sk = customer.c_current_hdemo_sk AND store_returns.sr_cdemo_sk = customer_demographics.cd_demo_sk AND store_returns.sr_cdemo_sk = customer.c_current_cdemo_sk AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date)
