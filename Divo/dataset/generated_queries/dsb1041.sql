--dsb_templates_query019_088_2eee297e-cd3d-3025-833e-413140958554.sql
--{"gen": "erase", "time": 1.5702378749847412, "template": "query019_088", "dataset": "dsb_templates", "rows": 100}
...
WHERE (item.i_category = 'Men' AND customer_address.ca_state = 'LA' AND customer.c_birth_month = 8 AND store_sales.ss_wholesale_cost BETWEEN 5 AND 25 AND store_sales.ss_item_sk = item.i_item_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND customer.c_current_addr_sk = customer_address.ca_address_sk AND store_sales.ss_store_sk = store.s_store_sk AND pg_catalog.substring(customer_address.ca_zip, 1, 5) <> pg_catalog.substring(store.s_zip, 1, 5))
