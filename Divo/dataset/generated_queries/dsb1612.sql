--dsb_templates_generated-5f0f5699-4051-4a38-99a2-7d35e0eeff28_3f059123-93cd-34fd-ab51-e57cfb772351.sql
--{"gen": "erase", "time": 0.4001009464263916, "template": "generated-5f0f5699-4051-4a38-99a2-7d35e0eeff28", "dataset": "dsb_templates", "rows": 1}
...
WHERE (d2.d_year = 1999 AND d2.d_moy = 11 AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND customer.c_current_cdemo_sk = customer_demographics.cd_demo_sk AND inventory.inv_date_sk = store_sales.ss_sold_date_sk AND inventory.inv_item_sk = store_sales.ss_item_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_returns.sr_item_sk = inventory.inv_item_sk AND inventory.inv_date_sk = d1.d_date_sk AND customer.c_customer_sk = store_returns.sr_customer_sk AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date AND inventory.inv_quantity_on_hand >= store_sales.ss_quantity)
 LIMIT 100