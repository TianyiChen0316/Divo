--dsb_templates_generated-4123fdef-494d-4a21-86c5-e05dc844651c_b0039732-3b24-3ee7-8004-791a0424200c.sql
--{"gen": "combine", "time": 3.3019378185272217, "template": "generated-4123fdef-494d-4a21-86c5-e05dc844651c", "dataset": "dsb_templates", "rows": 1}
...
WHERE (customer_address.ca_city = 'Farmington' AND date_dim.d_moy = 12 AND date_dim.d_year = 2002 AND item.i_category = 'Sports' AND store.s_state = 'MO' AND d2.d_moy = 5 AND d2.d_year = 2002 AND d3.d_moy BETWEEN 8 AND 8 + 2 AND d3.d_year = 2000 AND call_center.cc_class = 'large' AND ship_mode.sm_type = 'TWO DAY' AND warehouse.w_gmt_offset = -5 AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND customer.c_current_addr_sk = customer_address.ca_address_sk AND household_demographics.hd_demo_sk = customer.c_current_hdemo_sk AND date_dim.d_date_sk = store_sales.ss_sold_date_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_sales.ss_item_sk = item.i_item_sk AND store_returns.sr_cdemo_sk = customer.c_current_cdemo_sk AND store_returns.sr_item_sk = item.i_item_sk AND date_dim.d_date_sk = d1.d_date_sk AND customer.c_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_store_sk = store.s_store_sk AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_returns.sr_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = catalog_sales.cs_item_sk AND catalog_sales.cs_sold_date_sk = d3.d_date_sk AND store_sales.ss_customer_sk = catalog_sales.cs_bill_customer_sk AND catalog_sales.cs_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = store_sales.ss_item_sk AND catalog_sales.cs_bill_customer_sk = customer.c_customer_sk AND catalog_sales.cs_call_center_sk = call_center.cc_call_center_sk AND catalog_sales.cs_ship_mode_sk = ship_mode.sm_ship_mode_sk AND catalog_sales.cs_warehouse_sk = warehouse.w_warehouse_sk AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date)
 LIMIT 100