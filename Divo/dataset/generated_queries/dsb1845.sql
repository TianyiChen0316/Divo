--dsb_templates_query050_013_27c8d0ed-56b2-339f-ac86-39a713e1ebe7.sql
--{"gen": "combine", "time": 1.289625644683838, "template": "query050_013", "dataset": "dsb_templates", "rows": 1}
...
item AS item
WHERE (call_center.cc_class = 'large' AND catalog_sales.cs_list_price BETWEEN 124 AND 153 AND date_dim.d_month_seq BETWEEN 1187 AND 1187 + 23 AND warehouse.w_gmt_offset = -5 AND customer.c_birth_month = 4 AND customer_address.ca_state = 'CA' AND item.i_category = 'Women' AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND catalog_sales.cs_call_center_sk = call_center.cc_call_center_sk AND catalog_sales.cs_ship_date_sk = date_dim.d_date_sk AND catalog_sales.cs_warehouse_sk = warehouse.w_warehouse_sk AND customer.c_current_addr_sk = customer_address.ca_address_sk AND date_dim.d_date_sk = store_sales.ss_sold_date_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_sales.ss_item_sk = item.i_item_sk AND catalog_sales.cs_ship_date_sk = store_sales.ss_sold_date_sk AND store_returns.sr_item_sk = item.i_item_sk AND customer.c_customer_sk = store_returns.sr_customer_sk)
 LIMIT 100