--dsb_templates_query099_098_c4c9db49-df31-3d83-96d2-b497a0b6043e.sql
--{"gen": "combine", "time": 3.4372925758361816, "template": "query099_098", "dataset": "dsb_templates", "rows": 1}
...
WHERE (call_center.cc_class = 'large' AND catalog_sales.cs_list_price BETWEEN 20 AND 49 AND date_dim.d_month_seq BETWEEN 1196 AND 1196 + 23 AND ship_mode.sm_type = 'REGULAR' AND warehouse.w_gmt_offset = -5 AND d2.d_moy BETWEEN 2 AND 2 + 2 AND d2.d_year = 1999 AND catalog_sales.cs_call_center_sk = call_center.cc_call_center_sk AND catalog_sales.cs_ship_date_sk = date_dim.d_date_sk AND catalog_sales.cs_ship_mode_sk = ship_mode.sm_ship_mode_sk AND catalog_sales.cs_warehouse_sk = warehouse.w_warehouse_sk AND item.i_item_sk = store_sales.ss_item_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_returns.sr_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = catalog_sales.cs_item_sk AND store_sales.ss_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = store_sales.ss_item_sk)
