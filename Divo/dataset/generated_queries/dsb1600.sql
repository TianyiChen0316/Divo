--dsb_templates_query099_098_882a5554-7381-37fa-b931-74033199cf01.sql
--{"gen": "combine", "time": 3.1001381874084473, "template": "query099_098", "dataset": "dsb_templates", "rows": 3}
...
WHERE (call_center.cc_class = 'small' AND catalog_sales.cs_list_price BETWEEN 253 AND 282 AND date_dim.d_month_seq BETWEEN 1194 AND 1194 + 23 AND ship_mode.sm_type = 'EXPRESS' AND warehouse.w_gmt_offset = -5 AND customer.c_birth_month = 1 AND customer_address.ca_state = 'WA' AND item.i_category = 'Sports' AND store_sales.ss_wholesale_cost BETWEEN 31 AND 51 AND catalog_sales.cs_call_center_sk = call_center.cc_call_center_sk AND catalog_sales.cs_ship_date_sk = date_dim.d_date_sk AND catalog_sales.cs_ship_mode_sk = ship_mode.sm_ship_mode_sk AND catalog_sales.cs_warehouse_sk = warehouse.w_warehouse_sk AND customer.c_current_addr_sk = customer_address.ca_address_sk AND date_dim.d_date_sk = store_sales.ss_sold_date_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_sales.ss_item_sk = item.i_item_sk AND store_sales.ss_store_sk = store.s_store_sk AND catalog_sales.cs_ship_date_sk = store_sales.ss_sold_date_sk AND pg_catalog.substring(customer_address.ca_zip, 1, 5) <> pg_catalog.substring(store.s_zip, 1, 5))
