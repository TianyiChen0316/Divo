--dsb_templates_query050_013_e84f7a85-933d-36ca-b531-3990498a339b.sql
--{"gen": "combine", "time": 0.9966869354248047, "template": "query050_013", "dataset": "dsb_templates", "rows": 3}
SELECT store.s_city,
store.s_company_id,
store.s_county,
store.s_state,
...
date_dim AS d3,
item AS item
WHERE (d2.d_moy = 3 AND d2.d_year = 2002 AND d3.d_moy BETWEEN 10 AND 10 + 2 AND d3.d_year = 2002 AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_sales.ss_store_sk = store.s_store_sk AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND item.i_item_sk = store_sales.ss_item_sk AND store_returns.sr_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = catalog_sales.cs_item_sk AND catalog_sales.cs_sold_date_sk = d3.d_date_sk AND store_sales.ss_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = item.i_item_sk AND catalog_sales.cs_item_sk = store_sales.ss_item_sk AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date)
