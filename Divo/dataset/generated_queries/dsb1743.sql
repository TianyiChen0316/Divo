--dsb_templates_generated-f56bd3e6-0726-4a46-94f5-5cd279f1110f_2ba1d86c-93b2-3ddf-8452-116a49e82c3d.sql
--{"gen": "erase", "time": 0.36716270446777344, "template": "generated-f56bd3e6-0726-4a46-94f5-5cd279f1110f", "dataset": "dsb_templates", "rows": 1}
...
WHERE (customer_address.ca_city = 'Newtown' AND income_band.ib_lower_bound >= 64792 AND income_band.ib_upper_bound <= 63592 + 50000 AND date_dim.d_moy = 5 AND date_dim.d_year = 2001 AND store.s_state = 'KY' AND d2.d_moy = 9 AND d2.d_year = 2001 AND store_sales.ss_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_item_sk = store_returns.sr_item_sk AND store_sales.ss_sold_date_sk = d1.d_date_sk AND store_sales.ss_ticket_number = store_returns.sr_ticket_number AND customer.c_current_addr_sk = customer_address.ca_address_sk AND household_demographics.hd_demo_sk = customer.c_current_hdemo_sk AND income_band.ib_income_band_sk = household_demographics.hd_income_band_sk AND date_dim.d_date_sk = store_sales.ss_sold_date_sk AND store_sales.ss_customer_sk = customer.c_customer_sk AND store_returns.sr_cdemo_sk = customer.c_current_cdemo_sk AND date_dim.d_date_sk = d1.d_date_sk AND customer.c_customer_sk = store_returns.sr_customer_sk AND store_sales.ss_store_sk = store.s_store_sk AND store_returns.sr_returned_date_sk = d2.d_date_sk AND store_returns.sr_customer_sk = catalog_sales.cs_bill_customer_sk AND store_returns.sr_item_sk = catalog_sales.cs_item_sk AND store_sales.ss_customer_sk = catalog_sales.cs_bill_customer_sk AND catalog_sales.cs_item_sk = store_sales.ss_item_sk AND catalog_sales.cs_bill_customer_sk = customer.c_customer_sk AND d1.d_date BETWEEN d2.d_date - '120 day'::interval AND d2.d_date)
 LIMIT 100