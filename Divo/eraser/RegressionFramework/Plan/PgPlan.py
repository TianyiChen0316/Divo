import queue
import re
from .Plan import Plan, ScanPlanNode, PlanNode, FilterPlanNode, JoinPlanNode, ProjectPlanNode
from .PlanConfig import PgNodeConfig
    ...
    def to_node(cls, node_json, node_id=None):
        ...
        if node_type in PgNodeConfig.SCAN_TYPES:
            ...
            if node_type == 'Nested Loop':
                index_cond, relation_name, alias = None, None, None
                for child in node_json['Plans']:
                    if 'Index Cond' in child:
                        index_cond = child['Index Cond']
                        relation_name = child.get('Relation Name', None)
                        alias = child.get('Alias', None)
                        break
                    elif child['Node Type'] == 'Bitmap Heap Scan':
                        if 'Recheck Cond' in child:
                            index_cond = child['Recheck Cond']
                        elif 'Plans' in child and 'Index Cond' in child['Plans'][0]:
                            index_cond = child['Plans'][0]['Index Cond']
                        else:
                            continue
                        relation_name = child.get('Relation Name', None)
                        alias = child.get('Alias', None)
                        break
                if index_cond:
                    node_json['Index Cond'] = index_cond
                    if relation_name:
                        node_json['Relation Name'] = relation_name
                    if alias:
                        node_json['Alias'] = alias
...
class PgScanPlanNode(PgPlanNodeMixIn, ScanPlanNode):
    ...
    def _legacy_extract_filter_predicate(self):
    ...
    def extract_filter_predicate(self):
        ...
        # "(mi_idx.info > '10'::text)",
        predicates = []
        if "Filter" in self.node_json or "Index Cond" in self.node_json:
            iter = []
            if "Filter" in self.node_json:
                iter.append(self.node_json["Filter"])
            if "Index Cond" in self.node_json:
                iter.append(self.node_json["Index Cond"])
            iter = ' AND '.join(iter)
            env = sql_parser.ParserEnvironment()
            env.table_columns = database.schema.table_columns
            if "Relation Name" in self.node_json:
                table = self.node_json["Relation Name"]
                if "Alias" in self.node_json:
                    table = table + " as " + self.node_json["Alias"]
            else:
                table = '_dummy'
            entries = sql_parser.parse_select(f"select * from {table} where {iter}")
            clauses = []
            stack = [entries.where_clause]
            while stack:
                element = stack.pop()
                if isinstance(element, sql_parser.BoolExpr):
                    stack.extend(element.args)
                else:
                    clauses.append(element)
            for e in clauses:
                if isinstance(e, sql_parser.MathExpr):
                    lexpr, rexpr = e.lexpr, e.rexpr
                    if len(lexpr.concerned_aliases) == 1 and len(rexpr.concerned_aliases) == 0:
                        if not isinstance(lexpr, sql_parser.ColumnRef):
                            continue
                        col = str(lexpr)
                        if isinstance(rexpr, sql_parser.Const):
                            value = rexpr.value
                        elif isinstance(rexpr, sql_parser.TypeCast):
                            if isinstance(rexpr.arg, sql_parser.Const):
                                value = rexpr.arg.value
                                if rexpr.name.startswith('int'):
                                    value = int(value)
                                elif rexpr.name.startswith('float') or rexpr.name.startswith('numeric'):
                                    value = float(value)
                            else:
                                continue
                        else:
...
class PgJoinPlanNode(PgPlanNodeMixIn, JoinPlanNode):
    ...
    def _legacy_parse_join_key(self):
        node = self.node_json
        left_keys = []
        right_keys = []
    ...
    def _parse_join_key(self):
        ...
        if "Hash Cond" in node:
            ...
            join_str = node["Merge Cond"]
        elif "Join Filter" in node:
            join_str = node["Join Filter"]
        elif "Index Cond" in node:
            join_str = node["Index Cond"]
            if "Relation Name" in node:
                table = node["Relation Name"]
                if "Alias" in node:
                    table = table + " as " + node["Alias"]
        else:
            print('Warning: cannot find join condition')
            return 'null', 'null'
            #raise ValueError(f'cannot find join condition: {node}')
        env = sql_parser.ParserEnvironment()
        env.table_columns = database.schema.table_columns
        entries = sql_parser.parse_select(f"select * from {table} where {join_str}", env)
        clauses = []
        stack = [entries.where_clause]
        while stack:
            element = stack.pop()
        ...
        for e in clauses:
            if isinstance(e, sql_parser.MathExpr):
                if len(e.concerned_aliases) == 2 and len(e.concerned_columns) == 2:
                    left_column, right_column = tuple(e.concerned_columns)
                    return '.'.join(left_column), '.'.join(right_column)
        print('Warning: cannot find join condition')
        return 'null', 'null'
        #raise ValueError(f'cannot find join condition: {str(entries)}')
