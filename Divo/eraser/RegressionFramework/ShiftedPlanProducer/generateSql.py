...
from .joinOrderProducer import JoinOrderProducer, BruteForceJoinOrderProducer
from .sqlTemplate import SqlTemplate
from .statistic import Statistic
from ..config import sqls_for_each_join_key, histogram_bin_size, sqls_for_each_bin
...
class BaseSqlProducer:
    ...
    def _check_sql(self, sql, throw=False):
        # parsed = sqlparse.parse(sql)
        # if len(parsed) <= 0:
        #     raise RuntimeError
        timeout, database.timeout = database.timeout, 2000
        try:
            plan = database.plan(sql)[0][0][0]['Plan']
            if plan['Node Type'] == 'Result':
                # filter predicates have conflict, leading to an empty result
                return False
