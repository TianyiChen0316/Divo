...
from ..Plan.PgPlan import PgScanPlanNode
from ..Plan.Plan import PlanNode, ScanPlanNode, JoinPlanNode, Plan
from .histogram import Histogram
from ..StaticPlanGroup import StaticPlanGroup
from ..config import max_col_limit, histogram_bin_size
...
class StatisticDB:
    ...
    def build(self):
        ...
        if self.cache.exist():
            ...
            primary_keys = self._request_primary_keys()
            for table in tables:
                ...
                for i, col_type in enumerate(col_types):
                    ...
                    elif (table, col) in primary_keys:#"id" in col or "key" in col:
    ...
    def _request_primary_keys(self):
        query = """SELECT tc.table_name, c.column_name, c.data_type 
...
AND tc.table_name = c.table_name AND ccu.column_name = c.column_name WHERE constraint_type = 'PRIMARY KEY'"""
        ...
        return {(t, c): self._convert_col_type(d) for t, c, d in result}
    ...
    def _request_column_domain(self, table, column, col_type):
        if col_type in {"string", "bytea"}:
            return self._request_category_column_domain(table, column)
        elif col_type in {"numeric", "date", "boolean"}:
            return self._request_numeric_column_domain(table, column)
        return self._request_category_column_domain(table, column)
        #else:
        #    raise RuntimeError

    def _request_category_column_domain(self, table, column):
    ...
    def _convert_col_type(self, col_type):
        ...
        if col_type in {"numeric", "integer", "smallint", "bigint", "decimal", "real", "double precision", "smallserial", "serial", "bigserial", "money"}:
            return "numeric"
        elif col_type in {"character varying", "character", "text", "varchar", "char", "bpchar"}:
            return "string"
        elif col_type in {"bytea"}:
            return "bytea"
        elif col_type in {"date", "timestamp without time zone", "time without time zone", "timestamp", "time", "interval"}:
            return "date"
    ...
    def _run_query(self, query):
        ...
        return timer.time, res


class StatisticTrain:
    def __init__(self, training_set_name, db, enable_cache=True, logger=None):
    ...
    def build(self, plans, sqls):
        ...
        if self.cache.exist():
            ...
            self.logger(f"Loading statistics cache from {self.cache._get_absolute_path()}")
            res = self.cache.read()
            self.tables = res[0]
            ...
            # tables
            self.logger("Extract shifted tables")
            self.tables, self.alias_2_table = self._extract_tables(sqls)

            self.logger("Extract shifted join and predicates")
            for plan in plans:
                # join and predicates
                join_node_counts = [0]
            ...
            # projects
            self.logger("Extract shifted project cols")
            self.table_2_projectCols = self._extract_project_cols(sqls)

            self.logger(f"Saving statistics cache to {self.cache._get_absolute_path()}")
            self.cache.save(
    ...
    def _recurse_plan_collect_infos(self, node: PlanNode, join_node_counts: list):
        if isinstance(node, JoinPlanNode):
            ...
            if len(join_key) != 2 or join_key[0] == 'null' or join_key[1] == 'null':
                return
...
class StatisticTrainTpch(StatisticTrain):
    ...
    def __init__(self, training_set_name, db, logger=None):
        super().__init__(training_set_name, db, logger)
        self.alias_2_table = {
            "c": "customer",
            "l": "lineitem",
            "n": "nation",
...
from ....core.sql_featurizer import sql_parser, database
class StatisticTrainCustom(StatisticTrain):
    ...
    def _extract_project_cols(self, sqls):
        ...
        :return:
        """
        table_2_projectCols = defaultdict(set)
        for sql in sqls:
            parsed = sql_parser.parse_select(sql)
            targets = parsed.target_tables
            for target in targets:
                if target.element is None or len(target.element.concerned_columns) != 1:
                    continue
                if not isinstance(target.element, (sql_parser.ColumnRef, sql_parser.FuncCall)):
                    # to avoid bugs
                    continue
                table, column = tuple(target.element.concerned_columns)[0]
                # remove 'as' clause
                table_2_projectCols[table].add(str(target.element))
        return table_2_projectCols

    def _get_actual_tables_counts(self):
        return database.schema.size

class Statistic:
    def __init__(self, db, training_set_name, logger=None):
        self.statistic_db = StatisticDB(db)
        self.db = db
        if self.db == "imdb":
            self.statistic_train = StatisticTrainJob(training_set_name, db, logger=logger)
        elif self.db == "stats":
            self.statistic_train = StatisticTrainStats(training_set_name, db, logger=logger)
        elif self.db == "tpch":
            self.statistic_train = StatisticTrainTpch(training_set_name, db, logger=logger)
        else:
            self.statistic_train = StatisticTrainCustom(training_set_name, db, logger=logger)
            #raise RuntimeError
        if logger is None:
            logger = Logger('log', level=logging.INFO, to_stderr=True)
        elif isinstance(logger, str):
...
class StatisticTest:
    def __init__(self, statistic_train, structure_enable, join_enable, table_enable, filter_enable, logger=None):
