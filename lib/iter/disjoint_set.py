...
class DisjointSet(Set):
    ...
    def roots(self):
        return set(map(lambda x: x[0], filter(lambda x: x[0] == x[1], self.__set.items())))

    def groups(self, return_root=False):
        res = {}
        for k in tuple(self.__set.keys()):
            root = self.root(k)
            tup = res.get(root, None)
            if tup is None:
                tup = []
                res[root] = tup
            tup.append(k)
        ...
        return tuple(tuple(v) for v in res.values())
